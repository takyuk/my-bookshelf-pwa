...
module.exports=async()=>{
  ...
  async function failure(fetcher,stage,status,hint='unknown'){
    ...
    assert.equal(await response.text(),'NDL unavailable');
  ...
  console.log('PASS: relay diagnostic stages, status codes, exception classification, private-data exclusion and unchanged API responses');
};
