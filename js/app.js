...
const store = BookStorage.create(
  {
...
function setBusy(value) {
  ...
  document.dispatchEvent(
    new Event(value ? 'bookshelf-save-start' : 'bookshelf-idle')
  );
}
  ...
  try {
    ...
    const message =
      error.name === 'QuotaExceededError'
        ? '保存容量が不足しています。書き出しでバックアップし、ブラウザの空き容量を確認してください。'
        : error.message;
...
const list = BookList.create({
  $,
  getBooks: () => books,
  isBusy: () => busy,
  hasStorageError: () => storageReadFailed,
  store,
  covers,
  openEditor: (id) => editor.open(id),
  statusLabels
});
const editor = BookEditor.create({
  $,
  getBooks: () => books,
  isBusy: () => busy,
  hasStorageError: () => storageReadFailed,
...
['searchInput', 'statusFilter', 'ratingFilter', 'sortSelect'].forEach((id) =>
...
$('rescueBtn').addEventListener('click', () => {
  try {
    ...
    alert(
      '元データを救出できませんでした。ブラウザの保存設定を確認してください。'
    );
  }
});
window.addEventListener('storage', (event) => {
  if (event.key === BookStorage.key || event.key === null) refreshBooks();
});
window.addEventListener('focus', refreshBooks);
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'visible') refreshBooks();
});
$('commitVersion').textContent =
  globalThis.BOOKSHELF_VERSION?.label || '開発版（コミット情報なし）';
document.addEventListener('bookshelf-consent', refreshBooks);
refreshBooks();

$('showPhotoHelp').addEventListener('click', (event) => {
  event.preventDefault();
  $('photoHelpDialog').showModal();
});
$('closePhotoHelp').addEventListener('click', () =>
  $('photoHelpDialog').close()
);
