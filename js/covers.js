const BookCoverUI = {create({isBusy}){
  ...
  el('removeCover').addEventListener('click',()=>{if(isBusy())return;++generation;BookCorrection.cancel();clearPending();loading=false;coverId=null;record=null;clearPreview();el('coverStatus').textContent='保存すると書影を削除します。書籍情報は残ります。';});
