const BookCovers = (() => {
  ...
  el('correctCover').addEventListener('click',async()=>{
    if(!pendingFile||busy||loading)return;
  ...
  el('removeCover').addEventListener('click',()=>{if(busy)return;++generation;BookCorrection.cancel();clearPending();loading=false;coverId=null;record=null;clearPreview();el('coverStatus').textContent='保存すると書影を削除します。書籍情報は残ります。';});
