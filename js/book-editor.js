const BookEditor = {create({$, getBooks, isBusy, hasStorageError, store, covers, refreshBooks, saveBooks, render, uid, terms}){
  const dialog=$('bookDialog'), form=$('bookForm');
  let editSnapshot;
...
function closeDialog(){ if(!isBusy()) dialog.close(); }
...
form.addEventListener('submit', async (e)=>{
  e.preventDefault();
  if(isBusy()) return;
  if(covers.loading()){alert('画像の確認が終わるまでお待ちください。');return;}
  const books=getBooks();
