(() => {
  ...
  function apply(book, initial, id, overwrite) {
    if (
      !task.isCurrent(id) ||
      !formDialog.open ||
      BookISBN.normalize(isbnInput.value) !== book.isbn
    )
      return;
    let filled = 0;
    for (const name of [
      ...
      if (
        ...
        (overwrite || !input.value.trim())
      ) {
        input.value = book[name];
        filled++;
      }
    }
    const source = byId('catalogSource');
    source.hidden = !book.source;
    if (book.source) {
      source.href = book.source;
      source.textContent = '出典：国立国会図書館サーチ';
    }
    message(
      `${filled}項目を自動入力しました。内容を確認して「保存」を押してください。${book.issued && !book.publishedDate ? ` 刊行情報：${book.issued}（年月が確定しないため刊行年月の自動入力は省略）` : ''}${!overwrite ? ' 入力済みの項目は保持しています。' : ''}`
    );
    candidates.replaceChildren();
  }
  async function lookup() {
    ...
    if (!isbn) {
      message(
        '正しいISBNを入力してください（10桁または978・979から始まる13桁）。'
      );
      return;
    }
    ...
    const initial = Object.fromEntries(
      ['title', 'volume', 'author', 'publisher', 'publishedDate'].map(
        (name) => [name, byId(name).value]
      )
    );
    ...
    let timeout;
    try {
      const url = CatalogClient.endpoint();
      url.searchParams.set('isbn', isbn);
      lookupButton.disabled = true;
      message('国立国会図書館サーチで検索しています…');
      timeout = setTimeout(() => controller.abort(), 15000);
      const xml = await CatalogClient.fetchXml(url, controller.signal);
      if (!task.isCurrent(id) || !formDialog.open) return;
      const books = CatalogClient.parse(xml, isbn);
      if (!books.length) {
        message(errorText('NDL-NOT-FOUND'));
        return;
      }
      if (books.length === 1) {
        apply(books[0], initial, id, overwrite);
        return;
      }
      ...
      for (const book of books) {
        ...
        button.type = 'button';
        button.className = 'ghost catalog-choice';
        button.textContent = [
          book.title,
          book.volume,
          book.author,
        ...
        button.addEventListener('click', () =>
          apply(book, initial, id, overwrite)
        );
        candidates.appendChild(button);
      }
    } catch (error) {
      if (task.isCurrent(id))
        message(
          error.name === 'AbortError'
            ? errorText('APP-TIMEOUT')
            : error instanceof TypeError
              ? errorText('APP-CONNECTION')
              : error.message
        );
    } finally {
  ...
  const scanner = IsbnScanner.create({
    scanButton,
    panel,
    video,
    formDialog,
  ...
  byId('stopScanBtn').addEventListener('click', () => {
    stopCamera();
    message('読み取りを中止しました。');
  });
