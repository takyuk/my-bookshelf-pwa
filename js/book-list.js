...
const BookList = {create({$, getBooks, isBusy, hasStorageError, store, covers, openEditor, statusLabels}){
  const grid=$('bookGrid'), empty=$('emptyState');
function renderStats(){
  $('statTotal').textContent = getBooks().length;
  $('statUnread').textContent = getBooks().filter(b=>b.status==='unread').length;
  $('statReading').textContent = getBooks().filter(b=>b.status==='reading').length;
  $('statFinished').textContent = getBooks().filter(b=>b.status==='finished').length;
}
...
function render(){
  ...
  empty.textContent = getBooks().length ? '条件に一致する本がありません。' : '本がまだありません。「本を追加」から登録してください。';
  if(hasStorageError()){
  ...
  for(const b of list){
    ...
    node.querySelector('.card-hit').addEventListener('click',()=>openEditor(b.id));
