const BackupActions = {init({$, getBooks, isBusy, hasStorageError, store, storeKey, backup, refreshBooks, setBusy, saveBooks, render, download}){
$('exportBtn').addEventListener('click',async ()=>{
  ...
  try {
    ...
    const result=await navigator.locks.request(storeKey,async()=>{
      if(localStorage.getItem(storeKey)!==expected)throw new Error('別の画面で変更されました。再度書き出してください。');
...
$('importInput').addEventListener('change', async (e)=>{
  ...
  setBusy(true);
  try{
    const result=await backup.importFile(file),incoming=result.books;
    const warning = hasStorageError() ? '読み込めなかった元の保存データを置き換えます。先に「元データを救出」で保管してください。' : `現在の${getBooks().length}冊を置き換えます。`;
